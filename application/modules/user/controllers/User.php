...
class User extends CI_Controller {
    ...
    $ceksup= $this->session->userdata('supuser');
	    ...
	    if(trim($uri) == "view"){
	      $qry = $this->db->query("SELECT * FROM M_Admin WHERE AdminID = '$uri1'");
	       if ($qry->num_rows() > 0) {
	        $res = $qry->row();
	        $this->jcode($res);
	      }
		    ...
		    {
				$res = $this->db->query("SELECT AdminID FROM M_Admin WHERE AdminID = '".$_POST['id']."'");
					if ($res->num_rows() == 0) {

						//cek username
						$cek     = $this->db->query("SELECT UserName FROM M_Admin WHERE UserName = '".trim($_POST['username'])."'");
						if ($cek->num_rows() > 0) {
							$jeson['status']   = "Fail";
							$jeson['id']       = $_POST['id'];
						...
						$this->db->query("	
											...
											VALUES 
											( '".$_POST['nama']."', '".$_POST['birthday']."', '".$_POST['email']."', '".trim($_POST['username'])."', '".md5($_POST['password'])."',    
	      ...
	      if(trim($ceksup) == "Y"){
	      	$qry = $this->db->query("
	      							  SELECT * FROM M_Admin WHERE UserName = '$usernm'

	      							  UNION ALL 

	      							  SELECT * FROM M_Admin WHERE SuperUser ='N'
